...
We shall define a Bayesian classifier in the proceeding chapter, but we will not address the specifics of learning model parameters. For some insightful examples on how Bayesian classifiers learn parameters, the reader is encouraged to read David Heckerman's ``A Tutorial on Learning With Bayesian Networks," which is available online [Heckerman, 1996]. A URL is provided under the references. 
	...
	Let $\mathcal{B} \equiv \langle G, \Theta \rangle$, where $G = (V,E)$ is a directed acyclic graph, $V = \{V_0, V_1, \ldots, V_n \}$ is a set of $n + 1$ random variables, and $\Theta$ is a set of parameter estimates generated from some pre-classified data. Each parameter of $\Theta$ corresponds an arc in the graph structure $G$. That is, the parameters of our model are conditional probabilities. We say that $\mathcal{B}$ is a Bayesian Network.
...
\begin{tabular}{l |} 
 \toprule
  \textbf{Information} \\ % the first row of the table. Separate columns with ampersands and end the line with two backslashes. An environment begun in one cell will not carry over to adjacent rows.
  \midrule % another horizontal line
   	Whether or not the email was spam \\
 	The domain of the email's sender (e.g., @reed.edu) \\	
	A count of the number of hyperlinks in the body of the email \\ % another row
...
\label{bvf} % labels are useful when you have more than one table or figure in your document. See our online documentation for more on this.
\end{table}
	Note that we assume a human has pre-classified each email as either spam or not-spam.
	This is necessary for our classifier to learn the parameters of the model's network.
...
Our Na\"{i}ve Bayes classifier $\mathcal B = (G,\Theta)$ has a graph with the structure depicted in Figure 1.3. To complete the classifier, we would learn our model parameters from the training set. The parameters in our example would be $P(V_0 | V_1)$, $P(V_0 | V_2)$, and $P(V_0 | V_3)$. 
	...
	That being said, the NBC is not all-powerful. It works better than we would expect, but it is not guaranteed to be optimal when the feature variables of our model are not all independent. Hence, to improve upon the NBC, we need a means of finding the structure of our Bayesian classifier's network. 
	
	\subsection*{The Structure Space}
	Unfortunately, though, the size of the space of all possible network structures is {\em super-exponential} on the number of nodes in a network's graph. 
	Specifically, if $n$ is the number of nodes in a graph, the structure space increases in size at $2^{O(n^2\log{n})}$ [Friedman and Koeller, 2001].
	There are ways of decreasing the size of the structure space, one of which is using restricted Bayesian networks. 
	More importantly, though, we may choose between two modes of reasoning about model structure. 
...
\chapter{Monte Carlo with Markov Chains}
	...
	Andrey Markov used his eponymous chain only once in practice, to analyze the occurrence of vowels in a Pushkin poem [McGrayne, 2011]. According to legend, Enrico Fermi ran Markov chains in his head to combat insomnia, but we normal humans tend to reserve such calculations for computers. Though they can be treacherous to compute, the basic intuition behind Markov chains is, thankfully, fairly easy to grasp.
	...
	Suppose that Andrey can only assume four distinct colors: blue, green, yellow, or red. Hence, Andrey's state space $\mathcal S$ may be written as $\mathcal{S} = \{B, G, Y, R\}$. 
...
$$
	...
	%Andrei Markov's first application of his eponymous idea (Markov chains) was to a poem by Poeshkin. Over the next century, others found a wealth of uses for Markov chains. One recent example would be Google, whose idea of a "random surfer" informed their page ranking algorithm [cite the paper!].
		...
		We denote the number of steps before a chain hits $s_i$ an arbitrary number of times as $Z^{q}_i$, where $q$ is a positive integer. Clearly, $Z^{q}_i$ is always an integer, and $Z_i$ is simply shorthand for $Z^{1}_i$. We also define $Z^{0}_i = 0$. \\
		...
		\begin{proof}
		We begin our proof by recalling that the proportion of times we hit $s_j$ in the chain is the same regardless of $\theta^0$ as $n \rightarrow \infty$. Hence, without loss of generality, we assume $s_j$ is the initial state of the chain ($\theta^0 = s_j$).
		...
		\begin{center}
		 ...
		 \begin{center}
			 $E[U_{j}^q] = E[Z_j] = m_j$.
		 \end{center}
		Hence, we can consider $\frac{1}{n}\sum_{k=1}^{n}U_j^k$ an estimator for $m_j$. 
		Further, by the Strong Law of Large Numbers, we obtain the following as $n \rightarrow \infty$: 
		\begin{center}
		$
...
%		\end{center}
		Suppose we run an ergodic Markov chain $\theta^{(i)}$ for $n$ steps, where $n$ is large. The consequence of the Ergodic theorem for Markov chains is that $\frac{V_j(n)}{n}$ yields an estimate of $\frac{1}{m_j}$, which is equal to $\pi(s_j)$, since $\pi(s_j) * m_j = 1$.
	\section{Metropolis-Hastings for MCMC}
			Recall that in the case of GOFMC, we have a distribution $\pi$ from which we can easily simulate random draws, and we then estimate the expectation of a function over $\pi$ through random draws. Thus, to perform GOFMC, we must know the PDF for $\pi$. Sometimes, however, we are not so lucky as to be able to fully compute the PDF of our distribution of interest. This occurs frequently in high-dimensional Bayesian analyses, where the denominator of our posterior calculation can be a function integrated over hundreds (if not, more) dimensions. Thus, we seek an alternative way to simulate draws from $\pi$ when we only know how to calculate the marginal of $\pi$. For this, we can use a Markov chain.
	...
	In this scenario, we consider $\mathcal G$, the space of all possible graph structures for a Bayesian network $\mathcal B$.
		\subsection*{Being Bayesian}
			...
			As Bayesians, we are apt to treat $G$ as a random variable, since it is uncertain. 
			Hence, we write
			\begin{center}
			...
			\end{center}
			For the above summation, recall that $\mathcal G$ has dimension that grows super-exponentially with respect to the order of $V$, the number of variables in the model. 
		...
		\subsection*{The Proposal}
			...
			Then, if $(V_i, V_j) \in E$, $q$ proposes $G^{'} = (V,E - \{(V_i,V_j)\})$. 
			That is, $q$ proposes a removal of the arc from $E$. 
			...
			We denote the set of all directed acyclic graphs with one fewer or one greater arc than $G$ as $\mathcal{N}(G)$. 
