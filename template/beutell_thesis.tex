...
Our Na\"{i}ve Bayes classifier has a graph with the structure depicted in Figure 1.3. Of course, to complete our classifier, we would learn our model parameters from the training set.
	...
	We mentioned in the previous section that the Na\"{i}ve Bayes classifier is a surprisingly effective model, in spite of its simplifying assumption about the data. The performance of the NBC is surprising because in practice, feature variables are rarely independent, and ignoring the dependencies between them should give us biased probability estimates for $P(C | \vec{X})$. However, even if our probability estimates are incorrect, we can still predict classifications correctly [Domingos and Pazzani, 1996].
	...
	That being said, the NBC is not all-powerful. It works better than we would expect, but it is still prone to misclassification when the feature variables of our model are not all independent. Hence, to improve upon the NBC, we need a means of finding the structure of our Bayesian classifier's network. 
	...
	Specifically, if $n$ is the number of nodes in a graph, the structure space increases at $2^{O(n^2\log{n})}$ [Friedman and Koeller, 2001]. 
	When we have a large number of variables in our model, this immense size of the structure space is what motivates using restricted Bayesian networks. That is, doing so reduces the size of the structure space. 
...
$$
	...
	%Andrei Markov's first application of his eponymous idea (Markov chains) was to a poem by Poeshkin. Over the next century, others found a wealth of uses for Markov chains. One recent example would be Google, whose idea of a "random surfer" informed their page ranking algorithm [cite the paper!].
		...
		Proof of the Ergodic Theorem for Markov chains requires some preliminary results. Rigorous treatment of the following two results can be found in James Norris' {\em Markov Chains}. 
		
		First, the proportion of times we hit $s_j$ in an ergodic Markov chain $\theta^{(i)}$ is the same regardless of the initial state $\theta^0$ of the chain as $n$ goes to $\infty$. If we use the fact that ergodic Markov chains admit a unique stationary distribution, this result is almost immediate. 
		...
		\begin{center}
		 ...
		 = Z_j^r$. 
		 \end{center}
		 We consider the expectation of these quantities. 
		 We see that for all $q > 0$,
		\begin{center}
		...
		\begin{center}
		 ...
		 \begin{center}
			 $E[U_{j}^q] = E[Z_j] = m_j$.
		 \end{center}
		Hence, by the Strong Law of Large Numbers, we get: 
		\begin{center}
		$
...
%		\end{center}
		Suppose we run a Markov chain $\theta^(i)$ for $n$ steps, where $n$ is large. The consequence of the Ergodic theorem for Markov chains is that $V_j(n) / n$ yields an estimate of $P(s_j) = \pi(s_j)$, the probability of hitting state $s_j$ in the stationary distribution $\pi$ of the chain.  
	\section{MCMC}
		...
		\subsection*{Metropolis-Hastings}
			...
			Again, this is not wholly necessary, but it suffices for our purposes [Gamerman and Lopes, 2006].
